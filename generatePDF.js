...
const offset =4;
const underlineY = 107; // Adjust the Y position for the underline
 doc.setDrawColor(200, 0, 0);
doc.line(130 +offset , underlineY, 130 + textWidth+3, underlineY);
...
const boxWidth = 191 ; // Decrease the width of the box
const decreaseHeightFromBottom = 5; // Amount to decrease height from the bottom

// Increase the box height towards the top
const boxY = topStrengthsY - 25; // Y position for the box
const boxHeight = weaknessesYPosition - topStrengthsY + boxPadding * 3- decreaseHeightFromBottom; // Height to cover the entire content
...
doc.text(`Rater Name or Identifier: ${rater}`, 10,50); // Add the Rater value into the PDF
    ...
    doc.text(`Assessment Date: ${formattedDate}`, 10, 30);
                doc.setFont("helvetica", "bold");
                doc.setTextColor(0, 48, 143)
                doc.text("Sub-scale-continuous:", 10, 69);
...
doc.text(subtitle, (pageWidth - subtitleWidth) / 2, subtitleYPosition);
            ...
            html2canvas(document.querySelector("#assessmentForm"),{ scale: 1.5 }).then(canvas => {
                ...
                if (remainingHeight > 0) {
                    doc.addPage();
                    yPosition =3; // Reset Y position for the new page
